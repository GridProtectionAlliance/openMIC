...
namespace ConfigurationSetupUtility
{
    ...
    public partial class App : Application
    {
        ...
        public const string ApplicationExe = "openMIC.exe";
        public const string ApplicationConfig = "openMIC.exe.config";
        public const string Manager = "openMICManager";
        public const string ManagerExe = "openMICManager.exe";
        public const string ManagerConfig = "openMICManager.exe.config";
        public const string BaseSqliteConfig = "openMIC.db";
        public readonly static string SqliteConfigv2 = "openMIC" + DatabaseVersionSuffix + ".db";
        public const string SqliteSampleData = "openMIC-SampleDataSet.db";
        public const string SqliteInitialData = "openMIC-InitialDataSet.db";
        ...
        public App()
        {
            ...
            // Attempt to create an event log source for the openMIC Manager for authentication logging. This needs to be done
            // here since the CSU runs with administrative privileges and the openMIC Manager normally does not; also there is
            // a short system delay that exists before you can write to a new event log source after it is first created.
            try
            {
                string applicationName = "openMIC";

                // Create the event log source based on defined application name for openMIC if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");

                applicationName = "openMIC Manager";

                // Create the event log source based on defined application name for openMIC Manager if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");
            }
            catch (Exception ex)
            {
                m_errorLogger.Log(new InvalidOperationException(string.Format("Warning: failed to create or validate the event log source for the openMIC Manager: {0}", ex.Message), ex), false);
            }
        }
