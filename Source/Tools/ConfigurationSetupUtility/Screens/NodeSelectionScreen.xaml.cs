...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the openMIC to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("openMIC.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("openMICManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("openMIC.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("openMICManager.exe.config");
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\openMIC.exe.config";
        ...
        {
            return GetValueOfSystemSetting("openMIC.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the openMIC.
