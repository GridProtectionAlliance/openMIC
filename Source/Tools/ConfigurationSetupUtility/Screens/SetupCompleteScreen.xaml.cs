...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class SetupCompleteScreen : UserControl, IScreen
    {
        ...
        private ServiceController m_openMICServiceController;
        ...
        {
            InitializeComponent();
            InitializeopenMICServiceController();
            ...
            get
            {
                ...
                if (m_state != null)
                {
                    try
                    {
                        ...
                        try
                        {
                            ConfigurationFile openMICConfig = ConfigurationFile.Open("openMIC.exe.config");

                            // Some of the crypto settings elements were renamed for consistency, remove the old ones
                            CategorizedSettingsElementCollection cryptoSection = openMICConfig.Settings["cryptographyServices"];
                            cryptoSection.Remove("RetryDelayInterval");
                            cryptoSection.Remove("MaximumRetryAttempts");

                            // Example connection settings are updated between builds - remove the section and openMIC will add back the latest
                            openMICConfig.Settings.Remove("exampleConnectionSettings");

                            // Data publisher categories are now always lower case (such that code references are case insensitive)
                            openMICConfig.Settings.Remove("dataPublisher");

                            openMICConfig.Save(ConfigurationSaveMode.Full);
                        }
                        catch
                        {
                        ...
                        // If the user requested it, start or restart the openMIC service
                        if (m_serviceStartCheckBox.IsChecked.Value)
                        {
                            try
                            {
#if DEBUG
                                Process.Start("openMIC.exe");
#else
                                m_openMICServiceController.Start();
#endif
                            }
                            catch
                            {
                                MessageBox.Show("The configuration utility was unable to start openMIC service, you will need to manually start the service.", "Cannot Start Windows Service", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }
                        ...
                        if (m_managerStartCheckBox.IsChecked.Value)
                        {
                            ...
                            process.StartInfo.FileName = "http://localhost:8089/";
                            process.Start();
                        }
                    }
                    finally
                    {
                        if (m_openMICServiceController != null)
                            m_openMICServiceController.Close();
                    }
                }
        ...
        public Dictionary<string, object> State
        {
            ...
            set
            {
                ...
                if (Convert.ToBoolean(m_state["restarting"]))
                    m_serviceStartCheckBox.Content = "Restart the openMIC";
            }
        }
        ...
        // Initializes the openMIC service controller.
        private void InitializeopenMICServiceController()
        {
            ServiceController[] services = ServiceController.GetServices();
            m_openMICServiceController = services.SingleOrDefault(svc => string.Compare(svc.ServiceName, "openMIC", true) == 0);
        }

        // Initializes the state of the openMIC service checkbox.
        private void InitializeServiceCheckboxState()
        {
#if DEBUG
            bool serviceInstalled = File.Exists("openMIC.exe");
#else
            bool serviceInstalled = m_openMICServiceController != null;
#endif
            m_serviceStartCheckBox.IsChecked = serviceInstalled;
            m_serviceStartCheckBox.IsEnabled = serviceInstalled;
        }

        // Initializes the state of the openMIC Manager checkbox.
        private void InitializeManagerCheckboxState()
        {
            bool managerInstalled = File.Exists("openMICManager.exe");
        ...
        {
            string configFile = Directory.GetCurrentDirectory() + "\\openMIC.exe.config";
        ...
        private void ValidateNodeSettings()
        {
            ...
            try
            {
                ...
                if (m_state.TryGetValue("selectedNodeId", out selectedNodeId) && !string.IsNullOrWhiteSpace(selectedNodeId.ToNonNullString()))
                {
                    ...
                    {
                        nodeSettings.Add("RemoteStatusServerConnectionString", "server=localhost:8530;integratedSecurity=true;interface=0.0.0.0");
                    }
                    else
                    {
                    ...
                    if (!nodeSettings.TryGetValue("AlarmServiceUrl", out alarmServiceUrl))
                        nodeSettings.Add("AlarmServiceUrl", "http://localhost:5024/alarmservices");
                    ...
                    if (dataTable.Columns.Contains("TimeSeriesDataServiceUrl"))
                    {
                        ...
                        if (newConnection != null)
                        {
                            ...
                            foreach (DataRow row in dataTable.Rows)
                            {
                                ...
                                {
                                    remoteStatusServerSettings["dataPublisherPort"] = "6195";
