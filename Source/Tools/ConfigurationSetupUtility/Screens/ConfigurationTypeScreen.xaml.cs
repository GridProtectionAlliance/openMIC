...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ConfigurationTypeScreen : UserControl, IScreen
    {
        ...
        public ConfigurationTypeScreen()
        {
            ...
            try
            {
                // Determine if the openMIC is installed in this folder (could be stand-alone debug application or service), we check exe
                // instead of config file since config file can remain between installations. Since only the openMIC can be setup with a
                // XML or web service configuration, we disable these radio buttons for openMIC Manager only installations.
                string exeFileName = Directory.GetCurrentDirectory() + "\\openMIC.exe";

                if (!File.Exists(exeFileName))
                {
                    // If the openMIC is not be installed user may have chosen to only install the openMIC Manager on this system,
            ...
            {
                // Not failing if we cannot determine if openMIC is available...
            }
        }
