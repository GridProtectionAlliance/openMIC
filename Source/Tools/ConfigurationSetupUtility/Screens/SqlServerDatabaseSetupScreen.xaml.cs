            ...
            get
            {
                ...
                else
                {
                    ...
                    if (!hostIsLocal && m_createNewUserCheckBox.IsChecked != true && m_checkBoxIntegratedSecurity.IsChecked == true)
                    {
                        ...
                        {
                            const string failMessage = "Configuration Setup Utility has detected that the openMIC service account ({0}) is a local user, " +
                                "but the database server is not local. This user will not be able to log into the database using integrated security. " +
                                "Please either change the account under which the openMIC service runs, or choose to create a new database user.";
