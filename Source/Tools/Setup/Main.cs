...
namespace Setup
{
    public partial class Main : Form
    {
        ...
        private void buttonInstall_Click(object sender, EventArgs e)
        {
            ...
            RegistryKey openMICInstallKey;

            openMICInstallKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\" + ProductCode);

            // If key wasn't found, test for 32-bit virtualized location
            if (openMICInstallKey == null)
                openMICInstallKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\" + ProductCode);

            if (openMICInstallKey != null)
            {
                if (MessageBox.Show("An existing version of the openMIC is installed on this computer. Would you like to remove the existing version?\r\n\r\nCurrent configuration will be preserved.", "Previous Version Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        private bool RunSetup(SetupType type, bool closeOnSuccess)
        {
            ...
            // Install or uninstall openMIC
            Process openMICInstall = new Process();

            openMICInstall.StartInfo.FileName = "msiexec.exe";
            ...
            if (type == SetupType.Uninstall)
            {
                ...
                // Uninstall any version of the openMIC
                openMICInstall.StartInfo.Arguments = "/x " + ProductCode + " /qr";
            }
            else
            {
                // Install current version of the openMIC
                openMICInstall.StartInfo.Arguments = "/i Installers\\openMICSetup.msi";
            }

            openMICInstall.StartInfo.UseShellExecute = false;
            openMICInstall.StartInfo.CreateNoWindow = true;
            openMICInstall.Start();
            openMICInstall.WaitForExit();

            if (openMICInstall.ExitCode == 0)
            {
                // Run configuration setup utility post installation of openMIC, but not for uninstalls
                if (type == SetupType.Install)
                {
                    ...
                    installPath = AddPathSuffix(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Grid Protection Alliance\openMIC", "InstallPath", "").ToString().Trim());
                    targetBitSize = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Grid Protection Alliance\openMIC", "TargetBitSize", "64bit").ToString().Trim();
                    ...
                    {
                        MessageBox.Show("Setup program was not able to launch the openMIC Configuration Setup Utility due to an exception. You will need to run this program manually before starting the openMIC.\r\n\r\nError: " + ex.Message, "Configuration Setup Utility Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private void AttemptToStopKeyProcesses()
        {
            ...
            ServiceController openMICServiceController = null;

            try
            {
                foreach (ServiceController service in ServiceController.GetServices())
                {
                    if (string.Compare(service.ServiceName, "openMIC", true) == 0)
                    {
                        openMICServiceController = service;
                        break;
                    }
                }
            }
