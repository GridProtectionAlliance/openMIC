...
namespace ConfigurationSetupUtility
{
    ...
    public class SqlServerSetup
    {
        ...
        public void GrantDatabaseAccess(string loginName)
        {
            ...
            ExecuteStatement("IF DATABASE_PRINCIPAL_ID('openMICAdminRole') IS NULL CREATE ROLE [openMICAdminRole] AUTHORIZATION [dbo]");
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('{0}') IS NOT NULL AND DATABASE_PRINCIPAL_ID('openMICAdminRole') IS NOT NULL EXEC sp_addrolemember N'openMICAdminRole', N'{0}'", loginName));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('openMICAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datareader', N'openMICAdminRole'"));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('openMICAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datawriter', N'openMICAdminRole'"));
        }
                    ...
                    while ((object)line != null)
                    {
                        ...
                        else
                        {
                            if (trimLine.StartsWith("CREATE DATABASE") || trimLine.StartsWith("ALTER DATABASE") || trimLine.StartsWith("USE"))
                                line = line.Replace("openMIC", DatabaseName);
